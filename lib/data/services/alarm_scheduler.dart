...
Future<void> scheduleAlarmsForToday(int userId) async {
  ...
  for (var schedule in schedules) {
    ...
    final tz.TZDateTime computedAlarmTime = endTime.subtract(Duration(minutes: alarmOffset));

    // 알람 메시지에 스케줄명을 동적으로 포함 (예: 학교)
    String alarmBody = (alarmOffset == 0)
    ...
    if (computedAlarmTime.isAfter(now)) {
      await NotificationService().scheduleAlarm(
        ...
        body: alarmBody,
        scheduledDate: (alarmOffset == 0) ? endTime.toLocal() : computedAlarmTime.toLocal(),
      );
    } else if (alarmOffset == 0) {
      await NotificationService().scheduleAlarm(
        id: schedule.id ?? schedule.order,
        title: schedule.title,
        body: "종료시간에 알람이 울립니다. 스케줄 테이블에서 '${schedule.title}'을(를) 확인하세요.",
        scheduledDate: now.toLocal(),
      );
    }
  }
}
